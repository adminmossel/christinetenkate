...
export const BLOCK_LIBRARY = {
  ...
  image: { label: "Afbeelding", group: "Media", factory: () => ({ id: newId(), type: "image", url: "", alt: "", caption: "", align: "center", widthPercent: 100 }) },
  gallery: { label: "Galerij", group: "Media", factory: () => ({ id: newId(), type: "gallery", images: [] }) },
  file: { label: "Bestand / download", group: "Media", factory: () => ({ id: newId(), type: "file", url: "", name: "", label: "Bekijk het document", fileType: "", size: 0 }) },
...
export const HOME_ONLY_BLOCKS = {
  hero: { label: "Hero (bovenaan homepage)", group: "Homepage", factory: () => ({ id: newId(), type: "hero", eyebrow: "", title: "Welkom", lead: "", imageUrl: "", imageAlt: "", buttonText: "", buttonLink: null }) },
...
function linkButton(currentLink, onSet) {
  ...
  btn.textContent = currentLink?.value ? `Link: ${currentLink.value}` : "Link instellen";
  btn.addEventListener("click", async () => {
    const link = await openLinkPicker(currentLink);
    if (link !== undefined) {
      onSet(link);
      btn.textContent = link?.value ? `Link: ${link.value}` : "Link instellen";
    }
  });
  return btn;
}
...
function imagePreviewAndPicker(url, onPick) {
  const wrap = document.createElement("div");
  const trigger = document.createElement("div");
  trigger.className = "media-picker-trigger";

  function draw(currentUrl) {
    trigger.innerHTML = currentUrl
      ? `<img src="${currentUrl}" class="media-picker-preview">`
...
function editImage(block, patch) {
  const wrap = document.createElement("div");
  wrap.appendChild(imagePreviewAndPicker(block.url, (media) => {
    patch({ url: media.url, alt: block.alt || media.alt || "" });
  }));
...
function editGallery(block, patch) {
  ...
  function renderGrid() {
    ...
    (block.images || []).forEach((img, idx) => {
      ...
      cell.innerHTML = `<img src="${img.url}" style="width:100%;aspect-ratio:1;object-fit:cover;border-radius:4px;">`;
  ...
  addBtn.addEventListener("click", async () => {
    ...
    if (media) {
      ...
      block.images.push({ url: media.url, alt: media.alt || "" });
      patch({ images: block.images });
      renderGrid();
    }
  });
...
function editFile(block, patch) {
  ...
  trigger.textContent = block.url ? `Bestand: ${block.name}` : "Kies bestand uit mediabibliotheek";
  trigger.addEventListener("click", async () => {
    const media = await openMediaPicker({ accept: "any" });
    if (media) {
      patch({ url: media.url, name: media.name, fileType: media.contentType, size: media.size });
...
function editHero(block, patch) {
  ...
  wrap.appendChild(imagePreviewAndPicker(block.imageUrl, (media) => { patch({ imageUrl: media.url, imageAlt: block.imageAlt || media.alt || "" }); }));
